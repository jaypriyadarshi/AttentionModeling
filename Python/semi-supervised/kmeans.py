...
def compute_l2_dist(features, means):
...
def assign_label(features, means):
	scores = compute_l2_dist(features, means)
...
def calc_means(known_features, unknown_features, known_labels, predicted_labels):
	means = []
	for group in vars.groups:
...
def _run_kmeans(known_features, unknown_features, known_labels, means):
	...
	while True:
		...
		predicted_labels = assign_label(unknown_features, means)
		updated_means = calc_means(known_features, unknown_features, known_labels, predicted_labels)
...
if __name__ == '__main__':
	...
	print 'Accuracy: {0}'.format(accuracy(predicted_labels, unknown_labels))
